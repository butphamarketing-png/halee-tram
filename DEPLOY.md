...
Google Search Console vẫn cần submit sitemap **một lần thủ công**: `https://www.haleetram.com/sitemap.xml`. Sau đó mỗi lần Xuất bản, hệ thống tự **ping Google sitemap** (legacy ping API).
...
- Bot (Facebook, Zalo, Googlebot, Bingbot, …) → `/api/bot-render` inject meta + JSON-LD vào `index.html`
